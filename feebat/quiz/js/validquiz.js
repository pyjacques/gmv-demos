...
//// xr-environment // attribute causes estimated lighting to be used in the WebXR AR mode, rather than the lighting from the 3D mode, but does not affect Quick Look or Scene Viewer.
...
//   console.log('addEventListener: model-visibility')
//   console.log(evt)
// })
// modelViewer.addEventListener('ar-status', (event) => {
//   console.log('addEventListener: ar-status')
//   console.log(event)
//   console.log(event.detail.status)
// })

modelViewer.shadowRoot.querySelector('#default-exit-webxr-ar-button').addEventListener('click', function (evt) {
  console.log("Session AR exited: STOPING 'AR_animation' (loop).")
...
modelViewer.addEventListener('ar-status', (event) => {
  if (event.detail.status === 'session-started') {
    console.log("Session AR started: PLAYING 'AR_animation' (loop).")
...
modelViewer.addEventListener('load', (ev) => {
  ...
  if (!Boolean(modelViewer.canActivateAR)) {
    // Message A Dispo
  } else {
    // Message A NON Dispo
  }
...
async function animationLoop() {
  // hide hotspots
  modelViewer.animationName = 'AR_animation'
  await modelViewer.updateComplete
  modelViewer.timeScale = 1
  await modelViewer.play()
}
    ...
    if (currentHighlightedZone != undefined) {
      for (let m = 0; m < modelViewer.model.materials.length; m++) {
        if (modelViewer.model.materials[m].name == currentHighlightedZone) {
          ...
          modelViewer.model.materials[m].pbrMetallicRoughness.setBaseColorFactor([1.0, 0.6, 0.0, 0])
        }
      }
    }
  ...
  $('body').on('click', '.hotspot', function () {
    ...
    for (let j = 0; j < modelViewer.model.materials.length; j++) {
      if (modelViewer.model.materials[j].name == currentHighlightedZone) {
        ...
        modelViewer.model.materials[j].pbrMetallicRoughness.setBaseColorFactor([1.0, 0.6, 0.0, 0])
        ...
        modelViewer.model.materials[j].pbrMetallicRoughness.setBaseColorFactor([1.0, 0.6, 0.0, 0.5])
      }
    }
